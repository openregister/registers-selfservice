...
RSpec.describe ApiKeyRetentionJob, type: :job do
  it 'retains active users' do
    user_active = create(:user, created_at: 2.years.ago, user_type: 'api', api_key: SecureRandom.uuid)
    allow(ApiKeyRetentionJob::GoogleAnalyticsQueries).to receive(:get_active_api_keys_from_ga).and_return([user_active.api_key])
    ApiKeyRetentionJob.perform_now
    expect(User.count).to eq(1)
  end

  it 'deletes inactive users' do
    create(:user, created_at: 2.years.ago, user_type: 'api', api_key: SecureRandom.uuid)
    allow(ApiKeyRetentionJob::GoogleAnalyticsQueries).to receive(:get_active_api_keys_from_ga).and_return([])
    ApiKeyRetentionJob.perform_now
