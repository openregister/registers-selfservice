...
RSpec.describe ApiKeyInactiveDeletionJob, type: :job do
  it 'retains active users' do
    ...
    allow(GoogleAnalyticsQueries).to receive(:get_active_api_keys_from_ga).and_return([user_active.api_key])
  ...
  it 'deletes inactive users' do
    ...
    allow(GoogleAnalyticsQueries).to receive(:get_active_api_keys_from_ga).and_return([])
    ApiKeyInactiveDeletionJob.perform_now
